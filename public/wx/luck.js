...
//改变奖项
function changeLuck(){
	var option = $("#tagid option:selected").val();//请选择奖项
	...
	if(option>-1){//选中按奖项或者按人数
	var luckid = $("#luckid").val();//奖项活动id
	var meetingId = $("#meetingId").val();//会议id
	var url = "/admin/award?meetingId=" + meetingId;
	$.post(url,{"luckid":luckid,"luckTag_id":option},function(data){
		    //var data = JSON.parse(data);
		    console.log(data.map);
		    var signNum = getSignSum();//参加人数
		    var joinNum = signNum;
		    //给页面隐藏的奖品名称和按奖项抽奖人数赋值
		    $("#luckname").val(data.map.luck_name);//奖品名称
		    $("#tagNum").val(data.map.tagNum);//抽奖数量

		    if(option==0){
		    	var numExclude = $("#numExclude").val();//是否能重复抽奖
		    	if(numExclude==1){
				    joinNum = parseInt(signNum)-parseInt(data.map.num==undefined?0:data.map.num);
				    $("#may_num").html(joinNum>0?joinNum:0);//设置参加抽奖人数
		    	}else{
...
function getLuckUser(luckid,option){
	...
	var meetingId = $("#meetingId").val();//会议id
	var url = "/admin/awardGetuser?meetingId=" + meetingId;
    jQuery.ajax({
    	url: url,
    	data:luckTag,//,"type":type
    	type:"post",
    	async:false,//设置为同步
    	success:function(data){
		   // var data = JSON.parse(data);
		    $("#lotteryName").siblings().remove();
		    var length = data.luckList.length;
		    if(length>0){//奖项不为空
		    $(".lotteryDefault").hide();
            $.each(data.luckList,function(i,val){
...
function reset1(){
	$("#layer4").fadeIn();
	$("body").append("<div class=\"layerBlank\"></div>");
	$("#lotteryName").siblings().remove();//中奖名单移除
				$("#lucknum").text(0);//中奖人数归零
				var base = $("#may_num").html();//参与抽奖总人数
				$("div").remove(".layerBlank");//
				$("#luck_img").attr("src","/wxscreen/web/common/images/lotteryDefault.png");
				$("#luck_name").empty();
}
function reset(){
	...
	var option = $("#tagid option:selected").val();//--请选择奖项--值为-1
	if(option>-1){
	var alreadyNum = $("#luck_index").children("li").length;//中奖列表数量
	var tagExclude = $("#tagExclude").val();//按奖项抽奖是否可以重复中奖
	var numExclude = $("#numExclude").val();//按人数抽奖是否可以重复中奖
	var meetingId = $("#meetingId").val();//会议id
	var url = "/admin/awardReset?meetingId=" + meetingId;
	if(alreadyNum>0){
		$("#layer4").fadeIn();
		$("body").append("<div class=\"layerBlank\"></div>");
	    $("#layer4 :button:eq(0)").off().on("click",function(){
	    	var array = new Array();
			for(var i = 0;i < alreadyNum;i++){
				array[i] = $("#luck_index").children("li").eq(i).attr('id');
				console.log(array[i]);
			$.post(url,{"openid": array[i],"luckTagId":option},function(data){
		        //var data = JSON.parse(data);
			...
			//	alert("after:"+tagList);
				$("#luck_img").attr("src","/wx/lotteryDefault.png");
				$("#luck_name").empty();
...
var ret;
function startBtn(){
	var option = $("#tagid option:selected").val();//选择奖项
	if(option==-1){
		showLayer(1);
		return;
	}
	var base = $("#may_num").html();//参加抽奖人数
	if(parseInt(base)==0){
		showLayer(6);
		return ;
	}
	...
	$(".lotteryDefault").hide();//隐藏‘点击按钮，开始抽奖吧！’
	$("#startBtn").hide();//隐藏‘开始抽奖’
	$("#endBtn").show();
...
function change(ret){
	var openidList = getSignData();//获取签到墙列表
	var alldata = new Array();
	var newalldata = new Array();
	var option = $("#tagid option:selected").val();//获取抽奖类型
	var tagExclude = $("#tagExclude").val();//按奖项抽奖是否可以重复中奖
	var numExclude = $("#numExclude").val();//按人数抽奖是否可以重复中奖
	...
	var meetingId = $("#meetingId").val();//会议id
	var url = "/admin/luckUser?meetingId=" + meetingId;
	    jQuery.ajax({
	    	url: url,
	    	data:luckUser,
	    	type:"post",
	...
	var comment = $("#comment").val().trim();
		...
		idx = $("#luck_index li").length;
				$("#lotteryName").after('<li id="'+msg[3]+'"><p class="prize">'+comment+'</p><i class="sn">'+parseInt(idx+1)+'</i>'+
						'<p class="man"><img src="'+msg[2]+'" />'+msg[0]+'</p><i class="delLottery"  onclick=confirmLayer("'+msg[3]+'",'+123+');></i></li>');
				var lucknum = $("#luck_index").children("li").last().index();
				$("#lucknum").text(lucknum);//动态设置获奖人数
				idx++;
				$(".lotteryName li").on({
